...
The 3-running-instance-per-team cap is the only thing blocking mid.  Parallel
sessions hold all three slots and re-spin them as they expire, so the free window
...
spec = importlib.util.spec_from_file_location(
    "ctfmod", os.path.join(os.environ.get("CTF_HELPER_DIR") or os.path.expanduser("~/.local/share/ctf"), "ctf.py"))
...
def main():
    ...
    browning out, a GET can burn 30 s on a timeout, and a parallel session
    takes the slot in between.  A 409 when the cap is full is cheap and is the
