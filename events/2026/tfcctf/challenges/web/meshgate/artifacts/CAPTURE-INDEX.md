...
## Why the XXE line is a refutation

Three files together show it: `42-` shows the endpoint returns a title scraped from HTML, `44-` shows it treats a JSON document as opaque text rather than parsing it, and `46-` shows the scheme allow-list. The XML parse step for an external entity does not exist, and the non-http scheme to point one at does not exist either.
