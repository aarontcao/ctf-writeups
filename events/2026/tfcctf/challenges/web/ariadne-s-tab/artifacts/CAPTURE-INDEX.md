...
DNS still resolves for all three. The origin was torn down; the DNS records remain. The two app origins share one IP in AWS eu-central-1.

The challenge object in the platform API is `is_dynamic: false`. No per-team container ever started. The deployment was a single shared always-on one. That deployment is gone.
