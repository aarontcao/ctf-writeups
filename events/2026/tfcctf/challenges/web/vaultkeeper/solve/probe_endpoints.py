...
do not have a PHP-level REMOTE_ADDR check, so stage A alone reaches them:
`/api/peer_probe.php%3Fa.php?...` and `POST /api/webhook_test.php%3Fa.php`.
No redirector, tunnel, or 418, one request a probe.
...
# Stage A ACL bypass path for a script without a PHP-level loopback check.
def bp(script, query=""):
